...
| `No module named utils.world_model_wrapper` only inside Isaac Sim | Isaac imports another top-level `utils` package before V-JEPA, shadowing Meta's notebook utilities. | Import `notebooks.utils.world_model_wrapper` through the V-JEPA repository-qualified namespace. | Avoid generic top-level package names when integrating two large Python runtimes. |
